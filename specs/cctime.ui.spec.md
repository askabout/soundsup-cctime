...
Поддерживаются режимы: «День», «1 неделя», «2 недели», «3 недели», «4 недели», «5 недель». Переключение — через выпадающий список в подвале. По умолчанию — «День».

В режиме «День» отображается один день (текущее поведение). В многодневных режимах отображается горизонтальная сетка, где каждый день — группа колонок. Структура каждого дня идентична однодневному: заголовок даты → заголовки кабинетов → ячейки резервов.

В многодневном режиме неделя начинается с понедельника. При переключении в многодневный режим `currentDate` привязывается к понедельнику текущей недели.
...
- Многодневные режимы: `‹` / `›` — ±1 неделя, `«` / `»` — ±4 недели
...
- Свайп влево/вправо в режиме «День» переключает дату на следующий/предыдущий день. В многодневных режимах свайп отключён (используется горизонтальный скролл).
...
Отображаются:
- выпадающий список режима отображения (`select`): День, 1 неделя, 2 недели, 3 недели, 4 недели, 5 недель
