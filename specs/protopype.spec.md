...
Состоит из колонок:
- колонка времени (узкая, текст интервала может переноситься на несколько строк)
...
Если Web Share API не поддерживается браузером, текст копируется в буфер обмена с уведомлением пользователю.
...
- Если оба подтверждения (`clientConfirm` и `specialistConfirm`) установлены в `true`, вся ячейка резерва заливается зелёным цветом.
